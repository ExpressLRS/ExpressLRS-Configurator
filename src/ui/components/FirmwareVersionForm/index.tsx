import {Alert, AlertTitle, makeStyles, Tab, Tabs, TextField,} from '@material-ui/core';
import React, {FunctionComponent, useEffect, useState} from 'react';
import {Loader} from '../Loader';
import ShowAlerts from '../ShowAlerts';
import OctopusGitHubClient from '../../library/GitHubClient';
import Omnibox from '../Omnibox';
import {FirmwareSource, FirmwareVersionData} from '../../../main/handlers/BuildFirmwareHandler';
...
export const FirmwareVersionForm: FunctionComponent<FirmwareVersionCardProps> = (props) => {
  const {
    onChange,
    repositoryOwner,
    repositoryName,
  ...
  const [firmwareSource, setFirmwareSource] = useState<FirmwareSource>(data?.source || FirmwareSource.GitTag);
  const handleFirmwareSourceChange = (_event: React.SyntheticEvent, value: FirmwareSource) => {
    setFirmwareSource(value);
  };

  const [gitTags, setGitTags] = useState<string[]>([]);
  const [currentGitTag, setCurrentGitTag] = useState<string>(data?.gitTag || '');
  ...
  const [currentGitBranch, setCurrentGitBranch] = useState<string>(data?.gitBranch || '');
  ...
  useEffect(() => {
    ...
    switch (firmwareSource) {
      case FirmwareSource.GitTag:
        setLoading(true);
        new OctopusGitHubClient().loadTags(repositoryOwner, repositoryName).then((tags) => {
          setLoading(false);
          if (tags.length === 0) {
      ...
      case FirmwareSource.GitBranch:
        setLoading(true);
        new OctopusGitHubClient().loadBranches(repositoryOwner, repositoryName).then((branches) => {
          setLoading(false);
  ...
  return (
    <>
      <Tabs className={styles.tabs} defaultValue={FirmwareSource.GitTag} value={firmwareSource}
      ...
      {firmwareSource === FirmwareSource.GitTag && <>
        <div className={styles.tabContents}>
          {!loading && <Omnibox
            ...
            currentValue={currentGitTag === '' ? null : {label: currentGitTag, value: currentGitTag}}
            onChange={onGitTag}
          />}
        </div>
      </>}
      ...
      {firmwareSource === FirmwareSource.GitBranch && <>
        <Alert severity="warning" className={styles.dangerZone}>
          ...
          Use these sources only if you know what you are doing or was instructed by project developers
        </Alert>
        <div className={styles.tabContents}>
          {!loading && <Omnibox
            title="Git branches"
            options={gitBranches.map((branch) => ({label: branch, value: branch}))}
            currentValue={currentGitBranch === '' ? null : {label: currentGitBranch, value: currentGitBranch}}
      ...
      {firmwareSource === FirmwareSource.GitCommit && <>
        ...
        <div className={styles.tabContents}>
          <TextField id="git-commit-hash" label="Git commit hash" fullWidth value={gitCommit}
      ...
      {firmwareSource === FirmwareSource.Local && <>
        ...
        <div className={styles.tabContents}>
          <TextField id="local-path" label="Local path" fullWidth value={localPath} onChange={onLocalPath}/>
        </div>
      </>}
