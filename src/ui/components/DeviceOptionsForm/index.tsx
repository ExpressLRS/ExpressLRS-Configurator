...
export const DeviceOptionsForm: FunctionComponent<DeviceOptionsFormProps> = (props) => {
  const {
    target,
    categories,
    deviceOptions,
  ...
  const onOptionUpdate = (data: UserDefineOption) => {
    let updatedOptions = deviceOptions?.userDefineOptions.map((opt) => {
      if (opt.key === data.key) {
        return {
          ...data,
        };
  ...
  const onUserDefinesTxt = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    onChange({
      ...deviceOptions,
  ...
  const onUserDefinesMode = (_event: React.ChangeEvent<HTMLInputElement>, value: string) => {
    onChange({
      ...deviceOptions,
  ...
  return (
    <div>
      <FormControl component="fieldset" className={styles.userDefinesMode}>
        <RadioGroup row value={deviceOptions.userDefinesMode} onChange={onUserDefinesMode} defaultValue="top">
          <FormControlLabel
      ...
      {target === null && deviceOptions.userDefinesMode === UserDefinesMode.UserInterface && <Alert severity="info">
      ...
      {target !== null && categories !== null && deviceOptions.userDefinesMode === UserDefinesMode.UserInterface && <>
        <Grid container spacing={3}>
          <Grid item xs>
            {categories[UserDefineCategory.RegulatoryDomains]?.length > 0 && <>
              <Typography variant="h6" className={styles.categoryTitle}>Regulatory domains</Typography>
              <UserDefinesList
                options={deviceOptions.userDefineOptions}
                onChange={onOptionUpdate}
                whitelistKeys={categories[UserDefineCategory.RegulatoryDomains]}
              />
            </>}
            {categories[UserDefineCategory.BindingPhrase]?.length > 0 && <>
              ...
              <UserDefinesList
                options={deviceOptions.userDefineOptions}
                onChange={onOptionUpdate}
                whitelistKeys={categories[UserDefineCategory.BindingPhrase]}
              />
            </>}
            {categories[UserDefineCategory.HybridSwitches]?.length > 0 && <>
              ...
              <UserDefinesList
                options={deviceOptions.userDefineOptions}
                onChange={onOptionUpdate}
                whitelistKeys={categories[UserDefineCategory.HybridSwitches]}
              />
            </>}
            {categories[UserDefineCategory.CompatibilityOptions]?.length > 0 && <>
              ...
              <UserDefinesList
                options={deviceOptions.userDefineOptions}
                onChange={onOptionUpdate}
                whitelistKeys={categories[UserDefineCategory.CompatibilityOptions]}
              />
            </>}
          </Grid>
          ...
          <Grid item xs>
            {categories[UserDefineCategory.PerformanceOptions]?.length > 0 && <>
              ...
              <UserDefinesList
                options={deviceOptions.userDefineOptions}
                onChange={onOptionUpdate}
                whitelistKeys={categories[UserDefineCategory.PerformanceOptions]}
              />
            </>}
            {categories[UserDefineCategory.OtherOptions]?.length > 0 && <>
              ...
              <UserDefinesList
                options={deviceOptions.userDefineOptions}
                onChange={onOptionUpdate}
                whitelistKeys={categories[UserDefineCategory.OtherOptions]}
              />
            </>}
          </Grid>
        </Grid>
