...
./build-mac.sh -a            # auto-detect native architecture
...
Creates or refreshes the `mac-deps/` directory with symlinks pointing to the correct architecture-specific toolchain directories. These stable paths (`mac-deps/portable-python`, and `mac-deps/portable-git` on x64) are what `package.json` references in its `extraFiles` configuration. On arm64, only portable-python is symlinked — git is not bundled because the app uses `/usr/bin/git` from macOS.
...
**Fix:** `build-mac.sh` handles this automatically by running `rm -rf mac-deps || true` before creating fresh symlinks. No manual intervention needed — just re-run the build script.
...
3. Run a clean build from scratch: remove `mac-deps/`, then re-run `./build-mac.sh`
