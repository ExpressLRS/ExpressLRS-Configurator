...
elif [[ ${#ARGS[@]} -gt 0 ]]; then ARCH="${ARGS[0]}"; ARCH_SOURCE="from CLI"
...
# Only arm64 requires this because x86_64 portable-git (git-2.29.2)
# is a stable upstream release that doesn't change. arm64 portable tools are
...
if [[ "$ARCH" == "arm64" ]]; then
    ...
    if [ ! -d "$python_dir" ] || [ -z "$(ls -A "$python_dir")" ]; then
        echo "▶ Downloading missing portable dependencies for arm64..."
...
# Git is only bundled on x64; arm64 uses the system /usr/bin/git
if [[ "$ARCH" != "arm64" ]]; then
...
yarn build
echo "▶ Running electron-builder --mac --${ARCH}"
exec npx electron-builder --mac --"${ARCH}"
